...
// ---------- 创建默认单词库 ----------
void createDefaultWords() {
    FILE* fp = fopen(WORD_FILE, "w");
    if (!fp) {
        printf("无法创建单词文件！\n");
        return;
    }

    // 默认单词列表 - 使用const char*数组
    const char* defaultWords[][2] = {
        {"apple", "苹果"},
        {"book", "书"},
        {"cat", "猫"},
        {"dog", "狗"},
    ...
    int count = sizeof(defaultWords) / sizeof(defaultWords[0]);
    for (int i = 0; i < count; i++) {
        fprintf(fp, "%s,%s\n", defaultWords[i][0], defaultWords[i][1]);
    }
    fclose(fp);
    printf("已创建默认单词库，包含 %d 个单词。\n", count);
}
